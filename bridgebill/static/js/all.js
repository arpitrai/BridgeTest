...
$(document).ready(function(){
    $('p#record_bill_option button#record_bill_option_equal').click(function(){
        if ($(this).data('show')=='no') {
            if ($('p#record_bill_option button#record_bill_option_unequal').data('show') == 'yes') {
                $('p#record_bill_option button#record_bill_option_unequal').removeClass('selected');
                $('p#record_bill_option button#record_bill_option_unequal').data('show','no');
                $('form#unequal_split').hide();
            }
            $(this).addClass('selected');
            $(this).removeClass('unselected');
            $('p#record_bill_option button#record_bill_option_unequal').addClass('unselected');
            $('form#equal_split').show();
            //$(this).parent().addClass('selected_type');
            $(this).data('show', 'yes');
        }
        else {
            if ($('p#record_bill_option button#record_bill_option_unequal').data('show') == 'no') {
                $('p#record_bill_option button#record_bill_option_unequal').removeClass('unselected');
            }
            $(this).removeClass('selected');
            $(this).removeClass('unselected');
            $('form#equal_split').hide();
            //$(this).parent().removeClass('selected_type');
            $(this).data('show', 'no');
        }
    });

    $('p#record_bill_option button#record_bill_option_unequal').click(function(){
        if ($(this).data('show')=='no') {
            if ($('p#record_bill_option button#record_bill_option_equal').data('show') == 'yes') {
                $('p#record_bill_option button#record_bill_option_equal').removeClass('selected');
                $('p#record_bill_option button#record_bill_option_equal').data('show','no');
                $('form#equal_split').hide();
            }
            $(this).addClass('selected');
            $(this).removeClass('unselected');
            $('p#record_bill_option button#record_bill_option_equal').addClass('unselected');
            $('form#unequal_split').show();
            //$(this).parent().addClass('selected_type');
            $(this).data('show', 'yes');
        }
        else {
            if ($('p#record_bill_option button#record_bill_option_equal').data('show') == 'no') {
                $('p#record_bill_option button#record_bill_option_equal').removeClass('unselected');
            }
            $(this).removeClass('selected');
            $(this).removeClass('unselected');
            $('form#unequal_split').hide();
...
$(document).ready(function(){
    $('form#equal_split button.add_friend').click(function(){
        var html_fragment_1 = '\n' + '<li><input tabindex="-1" type="checkbox" checked="checked"' + 'name="people_new" class="people_new" value="people_new_' + i + '" />';
        ...
        var html_fragment_4 = '<input tabindex="-1" class="remove_friend" type="button" value="&#10006;" /></li>' + '\n';
...
$(document).ready(function(){
    $('form#equal_split ul.people').on('click', 'input.remove_friend', function(event){
        $(this).parent().remove();
        event.preventDefault();
    });
    $('form#equal_split ul.people').on('keyup', 'input.dynamic_add_in', function(event){
        // For Escape
        if (event.keyCode == 27) { 
            $(this).nextAll('input.remove_friend').click();
        }
    });
});
...
function validate_equal_split_save(event){
    ...
    if (input_checked_counter === 0) {
        $('span.friend_checked_equal_split').css("visibility", "visible");
        event.preventDefault();
    }
    else {
        $('span.friend_checked_equal_split').css("visibility", "hidden");
    };
...
$(document).ready(function(){
    $('form#unequal_split').on('change', 'select[id*=id_number_of_people]', function(event){
        var total_amount = Number($('form#unequal_split input#id_amount').val()) 
        var total_people = Number($('form#unequal_split input#id_total_people').val());
        if (!(isNaN(total_amount)) && !(isNaN(total_people))) {
            if (($(this).val() !== '') && ($(this).val() !== 'Other') && (total_amount !== 0) && (total_people !== 0)) {
                var borrower_amount = total_amount/total_people * Number($(this).val());
                borrower_amount = Math.round(borrower_amount*100)/100;
...
// Start - To re-calculate individual amount if total amount or number of people change
// For amount change
$(document).ready(function(){
    $('form#unequal_split').on('keyup', 'input#id_amount', function(event){
        var total_amount = Number($('form#unequal_split input#id_amount').val()) 
        var total_people = Number($('form#unequal_split input#id_total_people').val());
        if (!(isNaN(total_amount)) && !(isNaN(total_people))) {
            $('form#unequal_split select[id*=number_of_people]').each(function(){
                if (($(this).val() !== '') && ($(this).val() !== 'Other') && (total_amount !== 0) && (total_people !== 0)) {
                    var borrower_amount = total_amount/total_people * Number($(this).val());
                    borrower_amount = Math.round(borrower_amount*100)/100;
                    $(this).parent().nextAll('td.amount').children('input[id*=borrower_amount]').val(borrower_amount); 
                }
                else {
                    if ((total_amount === 0) || (total_people === 0)) {
                        $(this).parent().nextAll('td.amount').children('input[id*=borrower_amount]').val(''); 
                    }
                }
            });
        }
...
// For number of people change
$(document).ready(function(){
    $('form#unequal_split').on('keyup', 'input#id_total_people', function(event){
        var total_amount = Number($('form#unequal_split input#id_amount').val()) 
        var total_people = Number($('form#unequal_split input#id_total_people').val());
        if (!(isNaN(total_amount)) && !(isNaN(total_people))) {
            $('form#unequal_split select[id*=number_of_people]').each(function(){
...
// End - To re-calculate individual amount if total amount or number of people change

// Start - Add more friends or people on Un-Equal Split page 
var i=0;
$(document).ready(function(){
    $('form#unequal_split button.add_friend').click(function(){
        var html_fragment_1 = '\n' + '<tr><td class="name"><input type="hidden" name="people_new" class="people_new" value="x_people_new_' + i + '" />';
        ...
        var html_fragment_6 = '<td class="remove_table_row"><input tabindex="-1" class="remove_friend" type="button" value="&#10006;" /></td></tr>' + '\n';
...
$(document).ready(function(){
    $('form#unequal_split table#unequal_split_people_table').on('click', 'input.remove_friend', function(event){
        $(this).parent().parent().remove();
        event.preventDefault();
    });
    // For escape on name and email input fields 
    $('form#unequal_split table#unequal_split_people_table').on('keyup', 'input.dynamic_add_in', function(event){
        // For Escape
        if (event.keyCode == 27) { 
            $(this).parent().nextAll('td.remove_table_row').children('input.remove_friend').click();
        }
    });
    // For escape on amount field
    $('form#unequal_split table#unequal_split_people_table').on('keyup', 'input[id*=x_id_borrower_amount]', function(event){
        // For Escape
        if (event.keyCode == 27) { 
            $(this).parent().nextAll('td.remove_table_row').children('input.remove_friend').click();
        }
...
function validate_unequal_split_save(event){
    ...
    if (input_checked_counter === 0) {
        $('span.friend_checked_unequal_split').css("visibility", "visible");
        event.preventDefault();
    }
    else {
        $('span.friend_checked_unequal_split').css("visibility", "hidden");
    };
...
$(document).ready(function(){
    ...
    $('li input.record_bill_date').datepicker({ dateFormat: "dd/mm/yy", altField: "#id_xdate", altFormat: "dd-mm-yy" });
    $('div#content_one').on('focus', 'form#record_payment input#id_date', function(){
        $(this).datepicker({ dateFormat: "dd/mm/yy", altField: "#id_xdate", altFormat: "dd-mm-yy" });
    });
});
// End - Date Plugin

// Start - AJAX for Settle Payment page to get bills dynamically based on person chosen
$(document).ready(function(){
    $('p#settle_payment_option').on('change', 'select#id_person', function(){
        //$(this).parent().nextAll('span.error_django').hide();
        $(this).parent().parent().nextAll('form#record_payment').hide();
        if ($(this).val() != 'blank_element') {
            $(this).parent().nextAll('span.ajax_loading').show();
            $.ajax({
                type: 'GET',
                url: '/settle-payment-ajax/', 
                context: this,
                data: { 'person': $(this).val() },
                datatype: 'html',
                success: function(result) {
                    $(this).parent().nextAll('span.ajax_loading').hide();
                    if ($('form#record_payment').length > 0) {
                        $('form#record_payment').remove();
                    }
                    $(this).parent().parent().parent().append(result);
...
// End - AJAX for Settle Payment page to get bills dynamically based on person chosen
...
// Start - Table row select on Settle Payment page
$(document).ready(function(){
    $('div#content_one').on('click', 'table#record_payment_table tbody tr', function(event){
...
// If you check header checkbox, all following checkboxes in li get selected
$(document).ready(function(){
    $('div#content_one').on('click', 'table#record_payment_table thead tr th.check_bill input:checkbox', function(){
        if ($(this).is(':checked')) {
...
// End - Table row select on Settle Payment page

// Start - Settle Payment - Stop submit if no rows are selected
$(document).ready(function(){
    $('div#content_one').on('click', 'form#record_payment li.last button#save', function(event){
        var temp = 'false';
...
$(document).ready(function(){
    $('ul#my_friends').on({
        click: function(){
            ...
            var html_fragment_5 = '<span class="ajax_loading"><img src="' + STATIC_URL + 'images/ajax-loader.gif" alt="Processing" /></span>';
            var html_fragment = '\n' + html_fragment_1 + '\n' + html_fragment_2 + '\n' + html_fragment_3 + '\n' + html_fragment_4 + '\n' + html_fragment_5 + '\n';
...
$(document).ready(function(){
    $('ul.my_friends.add_remove_button button.add_friend').click(function(){
        ...
        var html_fragment_5 = '<span class="ajax_loading"><img src="' + STATIC_URL + 'images/ajax-loader.gif" alt="Processing" /></span></li>';
        var html_fragment = '\n' + html_fragment_1 + '\n' + html_fragment_2 + '\n' + html_fragment_3 + '\n' + html_fragment_4 + '\n' + html_fragment_5 + '\n';
