        ...
        if authentication_form.is_valid():
            ...
            if user is not None:
                ...
                try:
                    ...
                    try:
                        user_exists_as_fb_user = UserSocialAuth.objects.get(user=user_exists)
                        if user_exists.has_usable_password():
                            error = 'Wrong password. Try again. <br />Alternatively, since you\'ve also used Facebook Connect on BridgeBill, just click the Facebook Connect button above to login'
                        else:
                            error = 'You\'ve used Facebook Connect to create an account on BridgeBill. Click the Facebook Connect button above to login'
                    except:
                        error = 'Wrong password. Try again'
                except:
                    try:
                        user_exists_as_fb_user = UserSocialAuth.objects.get(user__email__iexact=username)
                        error = 'You\'ve used Facebook Connect to create an account on BridgeBill. Click the Facebook Connect button above to login'
                    except:
                        error = 'User doesn\'t exist. <br /> To create a new account, click <a href="/user/sign-up/">here</a> or click the Facebook Connect button above'
    else:
...
def user_signup(request):
    ...
    if request.method == 'POST': 
        ...
        user_filled_data = { 'firstname': firstname, 'username': username, 'password1': password1, 'password2': password2 }
        if firstname == '':
        ...
        if firstname == '' or username == '' or password1 == '' or password2 == '':
            error['general'] = 'Please fill all the fields below'
            return render_to_response('user-signup.html', { 'user_filled_data': user_filled_data, 'error': error, 'request': request }, context_instance=RequestContext(request))

        if len(firstname) <= 2:
            error['firstname'] = True
            error['general'] = 'Please enter a valid name'
            return render_to_response('user-signup.html', { 'user_filled_data': user_filled_data, 'error': error, 'request': request }, context_instance=RequestContext(request))
        ...
        if password1 != password2:
            error['password1'] = True
            error['password2'] = True
            error['general'] = 'The two passwords do not match'
            return render_to_response('user-signup.html', { 'user_filled_data': user_filled_data, 'error': error, 'request': request }, context_instance=RequestContext(request))
        ...
        try:
            ...
            try:
                user_exists_fb_user = UserSocialAuth.objects.get(user__email__iexact=username)
                error['general'] = 'You\'ve already used Facebook Connect to create an account on BridgeBill. Please click the Facebook Connect button above to login'
                return render_to_response('user-signup.html', { 'user_filled_data': user_filled_data, 'error': error, 'request': request }, context_instance=RequestContext(request))
            except:
        ...
        return render_to_response('user-signup.html', { 'error': error, 'request': request }, context_instance=RequestContext(request))
            ...
            if userfriend_object.friend_email != friend_email:
                try: 
                    ...
                    if duplicate_userfriend_object.friend_deleted == 'Y':
                        ...
                        if duplicate_userfriend_object.friend_email.lower() == friend_email.lower():
                            userfriend_object.friend_name = friend_name
                            userfriend_object.friend_email = friend_email
                            userfriend_object.save()
                            result = { 'friend_name': userfriend_object.friend_name, 'friend_email': userfriend_object.friend_email, 'userfriend_id': userfriend_object.userfriend_id }
        ...
        if old_password and new_password1 and new_password2:
            if request.user.check_password(old_password):
                if new_password1 == new_password2:
                    ...
                    request.user.save()
                    return HttpResponseRedirect('/my-profile/change-password-success/')
                else:
